...
const submitForm = async () => {
  ...
  try {
    await router.put(`/api/head-of-family/${props.headOfFamily.id}`, form.value, {
      onSuccess: () => {
    ...
    console.error('Error updating head of family:', error)
  } finally {
    loading.value = false
  }
}
    ...
    <main class="flex-1 p-6">
      <div class="flex flex-col gap-6">
        <div class="flex items-center justify-between">
          <h1 class="font-semibold text-2xl">Edit Kepala Rumah</h1>
          <Button 
            label="Kembali" 
